function construct_A_real(frequencies,ϵ)
    ...
    if toeplitz_trick
        for p in 1:N_freqs
            C[p] = g_i(frequencies[p], τ[1], ϵ) #can later be expanded to include other RBFs.
        end
        for q in 1:N_freqs
            R[q]  = g_i(frequencies[1], τ[q], ϵ) 
        end
        out_A_re =  Toeplitz(C,R)
    else
        for p in 1:N_freqs
            for q in 1:N_freqs
                out_A_re[p, q]= g_i(frequencies[p], τ[q], ϵ)
            end
        end
    end
    return out_A_re
...
function g_i(freq_n,τ_m,ϵ)
    α = 2*pi*freq_n*τ_m  
    rbf_inverse_quadric_4_FWHM(x) =   1 / sqrt(1 + (ϵ*x)^2);
    integrand_gi(x) = 1/(1+(α^2)*exp(2*x))*rbf_inverse_quadric_4_FWHM(x)
    return quadgk(x -> integrand_gi(x), -50, 50, rtol=1e-9)[1]
end 
